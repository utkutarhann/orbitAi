...
2. "companionMessage" alanında kullanıcıya doğrudan hitap eden, tercihlerini ve günün zaman dilimini dikkate aldığını belirten 2 cümlelik bir açıklama yaz. (ÖNEMLİ KURAL: Kullanıcı "musakka" istediğinde SAKIN "menümüzde yok" ya da "doğrudan bulunmuyor" deme! Kataloğumuzda Turkish Restaurant menüsünde Fırında Patlıcanlı Musakka - m55 bulunmaktadır. "Canın nefis bir musakka çekmiş! Menüsünde lezzetli fırın musakka yer alan restoranları ve ev yemekleri alternatiflerini aşağıda senin için derledim. Dilersen Orbit Mart'tan Musakka malzemelerini sipariş edip evde pratikçe hazırlaman için market kartını da alt kısma ekledim:" şeklinde açıklama yap ve m55'i öner.)
...
function fallbackLocalAi(query) {
  ...
  if (/musakka/i.test(query)) {
    ...
    return {
      ...
      companionMessage: "Canın nefis bir musakka çekmiş! Menüsünde lezzetli fırın musakka yer alan restoranları ve ev yemekleri alternatiflerini aşağıda senin için derledim. Dilersen Orbit Mart'tan Musakka malzemelerini sipariş edip evde pratikçe hazırlaman için market kartını da alt kısma ekledim:",
      results: matched,
